...
Тип зависних функција као такав постоји у језику \textsc{Agda} и није га потребно дефинисати. За фамилију типова $P$ над типом $X$, тип зависних функција $\prod_{(x : X)} P (x)$ у језику \textsc{Agda} записујемо као $(x : X) \to P~x$. \textsc{Agda} је веома изражајна и омогућава нам да дефинишемо синтаксу на коју смо навикли:
...
\end{code}
На крају треба конструисати елемент тиме $X$, за исти контекст. Како се у контексту налази одговарајућа конструкција, тј. $x : X$ је у контексту, конструкцију тривијално завршавамо:
\begin{code}
¬-inv-functor f ny x = ny (f x)
\end{code}
Тражену конструкицју о функторијалности дупле негације извршавамо интерактивно на сличан начин.
\begin{code}
